...
uint8_t internal_event_intercept(void)
{
    ...
    if (event_code == HCI_COMMAND_COMPLETE_EVT)
    {
        ...
        if (p_cb->int_cmd_rsp_pending > 0)
        {
            ...
            if (opcode == 0xFC77)
            {
                /* int_cmd_rd_idx minus 2 to the previous cmd 0xFC77 */
                if (p_cb->int_cmd_rd_idx < 2){
                    p_cb->int_cmd_rd_idx += INT_CMD_PKT_MAX_COUNT;
                }
                p_cb->int_cmd_rd_idx = (p_cb->int_cmd_rd_idx-2);
            }
    ...
    else if (event_code == HCI_COMMAND_STATUS_EVT)
    {
        ...
        if (p_cb->int_cmd_rsp_pending > 0)
        {
            ...
            if (opcode == p_cb->int_cmd[p_cb->int_cmd_rd_idx].opcode)
            {
                ...
                /* Check if received Status event is 0xFC77 (MTK_SET_UART) */
                if (opcode == 0xFC77)
                {
                    /* int_cmd_rd_idx plus 1 to handle the following cmd 0xFF */
                    p_cb->int_cmd_rd_idx = ((p_cb->int_cmd_rd_idx+1) & \
                                            INT_CMD_PKT_IDX_MASK);
                }
                return TRUE;
...
void hci_h4_send_msg(HC_BT_HDR *p_msg)
{
    ...
    /* Check if sending Command is 0xFCC0 */
    if ((event == MSG_STACK_TO_HC_HCI_CMD) && (p_msg->len == 3))
    {
        /* Get the opcode from the Cmd packet */
        STREAM_TO_UINT16 (opcode, p);
        if (opcode == 0xFCC0)
        {
            /* Send MTK wake up magic number 0xFF instead */
            bytes_sent = userial_write(event, &magic, 1);
            p = ((uint8_t *)(p_msg + 1)) + p_msg->offset - 1;
            goto send_complete;
        }
        ALOGE("[h4] send 0x%x", opcode);
...
uint8_t hci_h4_send_int_cmd(uint16_t opcode, HC_BT_HDR *p_buf, \
                                  tINT_CMD_CBACK p_cback)
{
	  ALOGI("hci_h4_send_int_cmd:%d", opcode);
