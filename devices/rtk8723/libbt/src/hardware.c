...
#define BT_CONFIG_DIRECTORY "/system/etc/firmware/rtl8723as/"
...
static const char *get_firmware_name()
{
	...
	sprintf(firmware_file_name, FIRMWARE_DIRECTORY"rlt8723a_chip_b_cut_bt40_fw");
...
uint32_t rtk_get_bt_config(unsigned char** config_buf, uint32_t* config_baud_rate)
{
	...
	{
		ALOGE("can't access bt config file:%s, errno:%d-%s\n", bt_config_file_name, errno, strerror(errno));
		return -1;
	}
